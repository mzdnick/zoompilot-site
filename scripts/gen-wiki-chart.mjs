...
const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${VIEW_W} ${VIEW_H}" role="img" aria-label="Available steering torque from the EPS across speed, up to 40 mph. The EPS can apply up to 1148 counts at neighborhood speeds, ramping down through nine measured points to 620 by about 32 mph. zoompilot tracks that measured ceiling; stock openpilot caps every command at one flat 800-count scale, about 44 percent less torque below the ramp.">
  ...
  <text x="${x(GAP_V) + 8}" y="${gapLabelY}" fill="${C.accent}" font-family='${C.mono}' font-size="10.5" letter-spacing="0.04em"><tspan x="${x(GAP_V) + 8}" dy="0">${GAP_PCT}% more torque</tspan><tspan x="${x(GAP_V) + 8}" dy="13">than stock</tspan></text>
